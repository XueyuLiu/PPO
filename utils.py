...
class GraphOptimizationEnv:
    ...
    def calculate_reward(self, operation):
        ...
        if mean_feature_pos < self.previous_feature_pos_mean:
            reward += 10 * (self.previous_feature_pos_mean - mean_feature_pos)
        else:
            reward -= 10 * (mean_feature_pos - self.previous_feature_pos_mean)

        if mean_feature_cross > self.previous_feature_cross_mean:
            reward += 10 * (mean_feature_cross - self.previous_feature_cross_mean)
        else:
            reward -= 10 * (self.previous_feature_cross_mean - mean_feature_cross)
